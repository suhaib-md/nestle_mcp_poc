...
# Removes all SonarQube resources created by deploy-sonarqube.sh and create-sonarqube-argocd-apps.sh
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --keep-namespace: Don't delete the SonarQube namespace"
            exit 0
            ;;
        *)
            shift
...
confirm_cleanup() {
    ...
    echo "⚠️  This will remove ALL SonarQube resources including:"
    echo "   - Helm releases"
...
cleanup_sonarqube() {
    ...
    if helm status $SONARQUBE_RELEASE_NAME -n $SONARQUBE_NAMESPACE --kube-context $cluster_name >/dev/null 2>&1; then
        helm uninstall $SONARQUBE_RELEASE_NAME -n $SONARQUBE_NAMESPACE --kube-context $cluster_name --timeout=300s
    ...
    local timeout=120
    local counter=0
    while kubectl get svc -n $SONARQUBE_NAMESPACE --context=$cluster_name 2>/dev/null | grep -q LoadBalancer 2>/dev/null; do
    ...
    kubectl delete svc --all -n $SONARQUBE_NAMESPACE --context=$cluster_name --timeout=60s >/dev/null 2>&1 || true
    
    # Remove PVCs (if any)
    print_info "Removing persistent volume claims..."
    kubectl delete pvc --all -n $SONARQUBE_NAMESPACE --context=$cluster_name --timeout=60s >/dev/null 2>&1 || true
    ...
    if [ "$KEEP_NAMESPACE" = "false" ]; then
        ...
        if kubectl get namespace $SONARQUBE_NAMESPACE --context=$cluster_name >/dev/null 2>&1; then
            kubectl delete namespace $SONARQUBE_NAMESPACE --context=$cluster_name --timeout=120s
    ...
    kubectl get all -n $SONARQUBE_NAMESPACE --context=$cluster_name 2>/dev/null | grep sonarqube | awk '{print $1}' | xargs -r kubectl delete --context=$cluster_name -n $SONARQUBE_NAMESPACE --force --grace-period=0 >/dev/null 2>&1 || true
    
    print_success "SonarQube cleanup completed for $cluster_name"
}
...
cleanup_local_files() {
    ...
    for file in sonarqube-credentials-*.txt; do
        if [ -f "$file" ]; then
            rm -f "$file"
    ...
    rm -f /tmp/sonarqube-values-*.yaml 2>/dev/null || true
...
verify_cleanup() {
    ...
    if [ "$KEEP_NAMESPACE" = "false" ] && kubectl get namespace $SONARQUBE_NAMESPACE --context=$cluster_name >/dev/null 2>&1; then
        print_warning "Namespace still exists"
        issues=$((issues + 1))
    fi
    
    # Check for any LoadBalancer services
    if kubectl get svc -A --context=$cluster_name 2>/dev/null | grep -q "sonarqube.*LoadBalancer" 2>/dev/null; then
...
main() {
    ...
    if [ "$TARGET_CLUSTER" = "east" ]; then
        cleanup_sonarqube $EAST_CLUSTER $EAST_REGION
        ...
        cleanup_sonarqube $WEST_CLUSTER $WEST_REGION
    ...
    if [ $failed_cleanups -eq 0 ]; then
        print_success "SonarQube cleanup completed successfully on all target clusters!"
    ...
    echo "kubectl get all -n $SONARQUBE_NAMESPACE --context=multi-region-eks-east"
    echo "kubectl get all -n $SONARQUBE_NAMESPACE --context=multi-region-eks-west"
